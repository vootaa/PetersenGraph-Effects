...
// Balanced cyberpunk grid - purple/magenta version, no blue
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Deep cyberpunk background gradient - replaced blues with purples
    float gradientFactor = p.y * 0.2 + 0.5;
    vec3 col = mix(
        vec3(0.04, 0.01, 0.05), // Bottom - deep purple/red
        vec3(0.09, 0.01, 0.08), // Top - magenta/purple
        gradientFactor
    );

    // Add dark cityscape silhouettes - purple tint instead of blue
    float cityHeight = 0.4 + sin(p.x * 10.0) * 0.05;
    float cityscape = smoothstep(0.0, 0.01, p.y + cityHeight);
    col = mix(col, vec3(0.12, 0.02, 0.10) * 0.5, 1.0 - cityscape);
    
    // Add distant city lights - warm amber instead of cool blue
    float cityLights = pow(noise(p * 40.0), 6.0) * (1.0 - cityscape) * 2.0;
    col += cityLights * vec3(1.0, 0.5, 0.2) * 0.2;
    
    // Cyberpunk grid - replaced cyan with magenta
    float grid = balancedCyberGrid(p);
    vec3 gridColor = vec3(0.7, 0.0, 0.5); // Magenta color instead of cyan
    col += grid * gridColor * 0.15;
    
    // Distortion visualization - enhanced magenta
    float warpVis = length(warpedP - p) * 12.0;
    col += warpVis * vec3(1.0, 0.0, 0.5) * 0.2; // Magenta distortion visualization
    
    // Multi-layered neon glow - removed blue, enhanced magenta/red/gold
    float neonMagenta = balancedNeonGlow(d, 0.01, 1.8, gTime);
    float neonRed = balancedNeonGlow(d, 0.02, 0.9, gTime + 0.5);
    float neonPurple = balancedNeonGlow(d, 0.03, 0.4, gTime + 1.0);
    float neonYellow = balancedNeonGlow(d, 0.045, 0.2, gTime + 1.5);
    
    // Neon colors - removed blue colors, enhanced reds/purples
    vec3 magentaColor = vec3(1.0, 0.0, 0.6);
    vec3 redColor = vec3(1.0, 0.2, 0.0);
    vec3 purpleColor = vec3(0.6, 0.0, 0.5);
    vec3 yellowColor = vec3(1.0, 0.8, 0.0);
    
    col += neonMagenta * magentaColor; // Hot magenta inner glow
    col += neonRed * redColor;         // Red middle glow
    col += neonPurple * purpleColor;   // Purple outer glow
    col += neonYellow * yellowColor;   // Yellow outermost glow
    
    // Chromatic aberration - removed blue, replaced with purple
    ...
    for(int i = 0; i < 3; i++) {
        ...
        // RGB color spectrum - replaced blue with purple/magenta
        vec3 spectrumColor;
        if(i == 0) spectrumColor = vec3(1.0, 0.0, 0.0);      // Red
        else if(i == 1) spectrumColor = vec3(1.0, 0.8, 0.0); // Yellow instead of green
        else spectrumColor = vec3(0.8, 0.0, 0.8);            // Magenta instead of blue
    ...
    // Main edge with magenta look
    float edgeWidth = 0.005;
    float edge = smoothstep(edgeWidth, 0.0, abs(d));
    col += edge * vec3(1.0, 0.7, 0.9) * 0.7;
    
    // Solid fill with digital pattern - more red/purple, no blue
    float solid = smoothstep(0.002, -0.002, d);
    vec3 fillColor = vec3(0.15, 0.02, 0.13);
    
    // Add circuit pattern to fill - purple instead of blue
    float circuitPat = circuitPattern(warpedP, d);
    vec3 circuitColor = vec3(0.4, 0.0, 0.3); // Purple color instead of blue
    fillColor += circuitPat * circuitColor * 0.3;
    
    col += solid * fillColor;
    
    // Electric sparks - golden yellow
    ...
    // Color grading for magenta/purple aesthetic
    col.r = pow(col.r, 0.85); // Enhance reds
    col.g = pow(col.g, 1.05); // Suppress greens
    col.b = pow(col.b * 0.8, 0.9); // Reduce blues, but keep purples
...
/** SHADERDATA
{
    "title": "Petersen Graph - Magenta Ripple Cyberpunk",
    "description": "Spacetime ripple effect with magenta/purple cyberpunk aesthetic (no blue)",
    "model": "plane"
}
*/